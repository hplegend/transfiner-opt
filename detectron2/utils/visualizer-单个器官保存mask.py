...
class VisImage:
    ...
    def save(self, filepath):
        ...
        """
        xxx = filepath.replace(".npy", ".jpg")
        print('ddddddddddddddddddddd'+xxx)
        self.fig.savefig(xxx)
...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions, image_name=None):
        """
    ...
    def overlay_instances(
        ...
        *,
        ...
        # draw bounding box if only has width and height
        # if boxes is not None and boxes.shape[1] == 4:
        #     for box in boxes :
        #         self.draw_box(box)

        # 写死的
        base_path = '/content/drive/My Drive/Colab Notebooks/transfiner/organ_seg/'
        image_name_split = image_name.replace('.npy', '')
        image_name_path = base_path + image_name_split
        if not os.path.exists(image_name_path):
            os.mkdir(image_name_path, 0o777)
        ...
        for i in range(num_instances):
            ...
            mask_file_path = image_name_path + '/' + image_name_split + '_#' + str(i) + '.jpg'
            if masks is not None:
    ...
    def draw_binary_mask(
        ...
        if True:
            ...
            new_empty_img = np.zeros((512, 512, 3), dtype="uint8")
            vis_mask = VisImage(new_empty_img)

            rgba = np.zeros(shape2d + (4,), dtype="float32")
            rgba[:, :, :3] = color
            rgba[:, :, 3] = (mask.mask == 1).astype("float32") * alpha
            has_valid_segment = True
            vis_mask.ax.imshow(rgba, extent=(0, self.output.width, self.output.height, 0))
            # 写入单个mask器官
            rgb, alpha = np.split(rgba, [3], axis=2)
            img_save = rgb.astype('uint8')
            # print(path + str(i) + ext)
            # cv2.imwrite(path + str(i) + ext, img_save)
            vis_mask.save(save_mask_path)
